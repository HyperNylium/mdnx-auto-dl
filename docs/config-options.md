...
| `FALLBACK_TO_ANY_DUB`              | `false`                                                                       | boolean       | When `true`, if none of the desired or backup dubs are available for an episode, it will fall back to downloading the first alphabetically available dub instead of skipping the episode. |
