...
class PLEX_API:
    def __init__(self) -> None:
        ...
        if self.server_url is None or self.server_url == "":
            log_manager.log("MEDIASERVER_URL is not set or empty. Please set it in config.json. Exiting...", level="debug")
            sys.exit(1)
        ...
        log_manager.log(f"PLEX API initialized with: URL: {self.server_url}")
        ...
        while time.time() < deadline:
            ...
            if self.pin_id and self.pin_code:
                ...
                if new_token:
                    ...
                    log_manager.log("Authorization completed. Token stored.")
                    return True

            if self.pin_started_at and (time.time() - self.pin_started_at > PLEX_PIN_TIMEOUT_SECONDS):
                log_manager.log("PIN timed out. Generating a new one.")
                self._clear_pin_state()
                self._create_and_log_pin()

            time.sleep(poll_interval)

        log_manager.log(f"Authorization timed out after {format_duration(max_wait_seconds)}.", level="debug")
        return False

    def scan_library(self) -> bool:
        if not self.server_url:
            log_manager.log("MEDIASERVER_URL not configured. Skipping scan.")
            return False

        if not self._verify_token(self.token):
            log_manager.log("No valid token yet.")
            return False

        if self.url_override:
            log_manager.log("MEDIASERVER_URL_OVERRIDE is true. Using whatever is in MEDIASERVER_URL for scan endpoint.")
            url = self.server_url
        else:
            log_manager.log("Using standard Plex scan URL for all libraries.")
        ...
        try:
            ...
            log_manager.log(f"Scan status={resp.status_code}", level="debug")
            resp.raise_for_status()
            log_manager.log("Scan triggered successfully.")
            return True
        except requests.HTTPError as e:
            status = e.response.status_code if e.response is not None else "unknown"
            if status == 401:
                log_manager.log("401 Unauthorized. Token invalid or lacks permission.", level="debug")
            else:
                log_manager.log(f"HTTP error: {e}", level="debug")
        except requests.RequestException as e:
            log_manager.log(f"Request failed: {e}", level="debug")
        return False
    ...
    def _verify_token(self, token) -> bool:
        ...
        try:
            ...
            log_manager.log(f"Verify token status={resp.status_code}", level="debug")
            return resp.status_code == 200
        except requests.RequestException as e:
            log_manager.log(f"Token verify error (network?): {e}", level="warning")
            return False
    ...
    def _create_and_log_pin(self) -> None:
        ...
        if not self.pin_url_logged:
            log_manager.log(f"Open this URL in a browser to authorize the app:\n{auth_url}")
    ...
    def _poll_pin_for_token_once(self, pin_id, code):
        try:
            ...
            log_manager.log(f"PIN poll error: {e}", level="debug")
            return None

    def _store_token(self, token) -> None:
        ok = update_app_config("MEDIASERVER_TOKEN", token)
        if not ok:
            log_manager.log("Failed to persist MEDIASERVER_TOKEN to config.", level="debug")
        self.token = token
...
class JELLYFIN_API:
    def __init__(self) -> None:
        ...
        if self.api_key is None or self.api_key == "":
            log_manager.log("MEDIASERVER_TOKEN (API key) is not set or empty. Please set it in config.json. Exiting...", level="debug")
            sys.exit(1)

        log_manager.log(f"Initialized (URL: {self.server_url})")
    ...
    def scan_library(self) -> bool:
        ...
        if not self.api_key:
            log_manager.log("MEDIASERVER_TOKEN (API key) not configured. Skipping scan.")
            return False

        url = f"{self.server_url}/Library/Refresh"
        try:
            resp = requests.post(url, params={"api_key": self.api_key}, timeout=30)
            log_manager.log(f"Scan URL: {resp.url}", level="debug")
            resp.raise_for_status()
            log_manager.log(f"Response: {resp.text}", level="debug")
            log_manager.log("Scan triggered successfully.")
            return True
        except requests.RequestException as e:
            log_manager.log(f"Error triggering scan: {e}", level="debug")
            return False
...
def _get_media_server():
    ...
    if server_type == "plex":
        ...
        log_manager.log("MEDIASERVER_TYPE is not set. Please set it to 'plex' or 'jellyfin' in config.json. Exiting...", level="debug")
        sys.exit(1)
    else:
        log_manager.log(f"Unsupported media server type: {server_type}. Supported: 'plex' or 'jellyfin'. Exiting...", level="debug")
        sys.exit(1)
