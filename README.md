...
In `app`, things are a little different. Anything that is by itself, for example, `TEMP_DIR` are global env vars (all modules can use said variable).\
...
- I was not able to figure out a stable way to download the Bento4-SDK and multi-download-nx. For now, both are download from my webserver. There are the URLs:
